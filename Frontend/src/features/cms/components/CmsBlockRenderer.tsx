...
import type { ContentBlock } from '../../adminCms/types'
  ...
  return (
    ...
    >
      {block.backgroundImage && (
        <img src={block.backgroundImage} alt='' className='absolute inset-0 h-full w-full object-cover opacity-20' />
      )}
      ...
      <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
        {block.items.map((item) => (
          <div key={item.id} className='landing-reveal-soft rounded-2xl border border-(--border) bg-(--surface-muted) p-4'>
            <p className='text-xs font-semibold uppercase tracking-[0.2em] text-(--text-muted)'>{item.icon}</p>
...
export default function CmsBlockRenderer({ block, index }: CmsBlockRendererProps) {
  if (!isContentBlock(block)) {
    return (
      <div className='landing-reveal rounded-3xl border border-(--border) bg-(--surface-base) p-6 text-sm text-(--text-muted)'>
  ...
  if (block.type === 'rich_text') {
    return <RichText block={block} index={index} />
  }
  if (block.type === 'feature_grid') {
    return <FeatureGrid block={block} index={index} />
  }
  ...
  if (block.type === 'cta_band') {
    return <CtaBand block={block} index={index} />
  }
